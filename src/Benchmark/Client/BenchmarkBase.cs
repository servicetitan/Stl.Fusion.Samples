...
public abstract class BenchmarkBase
{
    ...
    protected Dictionary<string, Counter> Counters { get; set; } = new();
    ...
    protected virtual async Task Run(TimeSpan duration, CancellationToken cancellationToken)
    {
        ...
        var tasks = Enumerable.Range(0, ConcurrencyLevel)
            .Select(async i => {
                ...
                return await RunWorker(i, duration, cancellationToken).ConfigureAwait(false);
            })
            .ToArray();
    ...
    protected abstract Task<Dictionary<string, Counter>> RunWorker(
        int workerId, TimeSpan duration,
